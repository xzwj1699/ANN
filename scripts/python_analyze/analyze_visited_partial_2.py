...
def in_area_1(x, y):
    return y <= x * 0.7 + 15000 and y >= x * 0.7 + 7000

def in_area_2(x, y):
    return y <= x * 0.7 + 7000 and y >= x * 0.7 

if __name__ == "__main__":
    f = open("../../build/visited_20")
    lines = f.readlines()
    datas = []
    hit = 0
    miss = 0
    ...
    upper_bounds = [x * 0.7 + 15000 for x in range(len(block_ids))]
    middle_bounds = [x * 0.7 + 7000 for x in range(len(block_ids))]
    middle_bounds_2 = [x * 0.7 for x in range(len(block_ids))]
    lower_bounds = [x * 0.7 - 6000 for x in range(len(block_ids))]
